...
import type { PublisherResponse, TitleResponse } from "@/lib/api/api.schemas"
import { type TitleCardProps } from "@/components/title-card"

const PUBLISHER_LOOKUP_PAGE_SIZE = 24
...
const resolvePublisher = async (param: string) => {
  ...
  // 1. Try to get by ID
  const directMatch = await getPublishers()
    .getPublisher(normalizedParam)
    .catch(() => null)
  ...
  const searchResult = await getPublishers()
    .searchPublishers({
      search: normalizedParam,
      size: PUBLISHER_LOOKUP_PAGE_SIZE,
    })
    .catch(() => null)
...
export default function PublisherPage() {
  const params = useParams<{ publisherParam: string | string[] }>()
  const publisherParam = normalizeParam(params?.publisherParam)

  const [publisher, setPublisher] = useState<PublisherResponse | null>(null)
  const [titles, setTitles] = useState<TitleCardProps[]>([])
  const [isLoading, setIsLoading] = useState(() => Boolean(publisherParam))
  ...
  useEffect(() => {
    ...
    const loadPage = async () => {
      ...
      const resolvedPublisher = await resolvePublisher(publisherParam)
      ...
      if (!resolvedPublisher) {
        setPublisher(null)
        setTitles([])
        setErrorText("Издатель не найден")
        setIsLoading(false)
        return
      }
      ...
      const t = await getTitles()
        .searchTitles({
          publisherId: resolvedPublisher.id,
    ...
    return (
      <div className="mx-auto flex min-h-[70vh] w-full max-w-lg flex-col items-center justify-center gap-4 px-4 py-10 text-center sm:px-6">
        <Badge variant="outline" className="rounded-full px-3 py-1 text-xs">
          /publishers/{publisherParam || "unknown"}
        </Badge>
        ...
        <div className="space-y-6">
          ...
          <section className="relative overflow-hidden rounded-[2rem] border border-border/70 bg-card/90 p-5 shadow-sm backdrop-blur xl:p-8">
            ...
            <div className="relative grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
              <div className="mx-auto w-full max-w-65 lg:mx-0">
                <div className="relative aspect-square overflow-hidden rounded-2xl border border-border/70 bg-muted shadow-2xl shadow-primary/10">
                  <MediaImage
                    mediaId={publisher.logoMediaId}
                    alt={publisher.name || "Логотип"}
                    fill
                    className="object-contain"
                    fallback={
                      <div className="flex h-full items-center justify-center bg-muted text-muted-foreground">
                        <HugeiconsIcon icon={UserGroupIcon} className="size-16 opacity-30" />
                      </div>
                    }
                  />
                </div>
              ...
              <div className="space-y-5 flex flex-col justify-center">
                ...
                <p className="max-w-3xl text-sm leading-7 text-muted-foreground sm:text-base whitespace-pre-wrap">
                  {publisher.description || "Описание издателя пока не добавлено."}
                </p>
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {publisher.countryIsoCode ? (
                    <DetailTile
                      icon={Globe02Icon}
                      label="Страна"
                      value={publisher.countryIsoCode}
                    />
                  ) : null}
                  {(publisher.websiteUrls && publisher.websiteUrls.length > 0) ? (
                    <DetailTile
                      icon={Link01Icon}
                      label="Ссылки"
                      value={
                        <div className="flex flex-col gap-1">
                          {publisher.websiteUrls.map((url) => (
                            <a
                              key={url}
                              href={url}
                              target="_blank"
          ...
          <Card className="border border-border/70 bg-card/90 shadow-sm">
            <CardHeader className="border-b border-border/70">
              <CardTitle>Тайтлы издателя</CardTitle>
              <CardDescription>
                Список релизов, выпущенных данным издателем.
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              <TitleCardGrid
                ...
                emptyText="У этого издателя еще нет добавленных тайтлов."
              />
            </CardContent>
          </Card>
        </div>
